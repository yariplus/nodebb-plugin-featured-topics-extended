...
exports.renderFeaturedTopicsNews = async (widget) => {
  const {slug, sort, sorted, max, sortby, template} = widget.data
  const {uid} = widget
  ...
  let data = await controllers.parseFeaturedPageTopics(template, topics, 1, false, false, false, {req: widget.req, carousel: widget.data.carousel})
...
async function getWidgetTopics (widget) {
  const {slug, sort, sorted, max} = widget.data
  const {uid} = widget
...
async function getRenderWidgetTopics (template, widget) {
  if (!widget.req.params.wnum) widget.req.params.wnum = 0
  widget.req.params.wnum++

  let renderWidgetTopics = async (topics) => {
    widget.data.topics = topics
    widget.data.config = {relative_path: nconf.get('relative_path')}
    widget.data.fteWidget = widget.req.params.wnum
